...
#+END_SRC
*** check features
See the variables =system-configuration-features= and
...
#+begin_src bash
  ...
  ./autogen.sh
  ./configure --with-ns --disable-silent-rules --without-x --with-gnutls --with-xml2 --with-json --with-modules --without-dbus --without-imagemagick
  make -j4
