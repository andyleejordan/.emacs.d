...
nature. This command is =isearch=. Using [[isearch]] to move within a
...
search will push your starting location onto the mark ring and drop
...
Switch buffers =C-x b=, and list them with [[ibuffer]] on =C-x C-b=.
...
[[isearch]]. It lists all the occurrences of the search in an overview
